...
function Unauthorized({ history }: { history: any }) {
  return (
    <div className="max-w-screen-xl mx-auto w-full flex flex-col items-center pt-24">
      ...
      <div className="flex flex-row">
        <button className="rounded-md bg-gray-600 text-white font-bold p-2 mr-2" onClick={() => history.push("/login")}>
          로그인
        </button>
        <button className="rounded-md bg-gray-600 text-white font-bold p-2" onClick={() => history.push("/")}>
          메인 페이지로
        </button>
      </div>
    </div>
